    ...
    <>
      <div className="grid gap-6 row-gap-5 lg:grid-cols-3">
        {projectList.map((project, idx) => (
          <div key={project?.name || idx} className="mb-8" >
            <Image
              className=" mb-6 rounded shadow-lg lg:h-80 xl:h-96 transform hover:scale-125 transition duration-700 ease-out"
              src={project?.img || ""}
              alt={project?.alt || ""}
              width={700}
              height={500}
            ...
            <p className="text-gray-700">
              {truncateText(project?.desc_short, 400)}

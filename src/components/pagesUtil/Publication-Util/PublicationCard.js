...
const PublicationCard = () => {
    return (
        ...
        {publications.map((publication, idx) =>(
            ...
            <p className="text-base leading-relaxed px-4"><span className="font-semibold mr-4 text-mantis-700">Available from:</span> <a href={publication?.link || ""} className="text-blue-700">{publication?.link || ""}</a></p>
        </div>
        ))}
        </>
    )
